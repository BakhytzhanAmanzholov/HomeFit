...
def curl():
    ...
    str_base = str(video_base64)

    video_base64_new = str_base[2:]
    video_base64_new_new = video_base64_new[:-2]
    video_bytes = base64.b64decode(video_base64_new_new)
...
def squats():
    ...
    video_base64_new = str_base[2:]
    # video_base64_new_new = video_base64_new[:-2]
    video_bytes = base64.b64decode(video_base64_new)
    ...
    while True:
        ...
        if len(lmList) != 0:
            ...
            knee_ankle_color = (0, 255, 0) if not knee_beyond_foot else (
            0, 0, 255)  # Green if not beyond, red if beyond
...
@app.route('/abs_legs', methods=['POST'])
def abs_legs():
    video_base64 = request.json['video']
    str_base = str(video_base64)
...
@app.route('/lateral_raise', methods=['POST'])
def lateral_raise():
    video_base64 = request.json['video']
    str_base = str(video_base64)
    ...
    while True:
        ...
        if len(lmList) != 0:
            ...
            per = np.interp(angle, (260, 340),
                            (0, 100))  # Change these values according to the expected arm raise angle
